...
class AuthController extends Controller
{
    ...
    public function ajaxLogin()
    {
        if ($this->request->exists('post')) {
            ...
            if ($data['csrftoken'] && $token->validateToken($data['csrftoken'], $data['frm_name'])) {
                $model = self::$container->load([AuthManager::class => []])->Auth->assign($data);
                    ...
                    if ($user && (int)$user[0]->count() === 1) {
                        if ($user[0]->verified) {
                            if ($user[1] <= MAX_LOGIN_ATTEMPTS_PER_HOUR) {
                                $loginAtempt = self::$container->load([LoginAttemptsManager::class => []])->LoginAttempts;
                                if (password_verify($data['password'], $user[0]->password)) {
                                    $remember = (isset($data['rem']) && $data['rem'] === 'on') ? true : false;
                                    if ($user[0]->login($remember)) {
                                        $loginAtempt->delete($user[0]->userID);
                                        if (isset($data['checkout']) && $data['checkout'] == 'checkout') {
                                            if (!$this->session->exists(CHECKOUT_PROCESS_NAME)) {
                                                $this->session->set(CHECKOUT_PROCESS_NAME, true);
                                                $msg = 'checkout';
                ...
                if ($this->model_instance->validationPasses()) {
                    ...
                    if ($user && (int)$user[0]->count() === 1) {
                        if ($user[1] < MAX_PW_RESET_REQUESTS_PER_DAY) {
                            $request = self::$container->load([UsersRequestsManager::class => []])->UsersRequests;
                            ...
                            if ($lastID = $request->save()['saveID']->get_lastID()) {
                                $msg = '<a href="http://localhost/kngell/users/resetpassword/' . $lastID . '/' . self::$container->load([Token::class => []])->Token->urlSafeEncode($code) . '">Click to reset your password.</a>';
    ...
    {
        if ($this->request->exists('post')) {
            if ($userdata = self::$container->load([AuthManager::class => []])->Auth->rememberMe_checker()) {
    ...
    public function ajaxRegister()
    {
        if ($this->request->exists('post')) {
            $data = $this->request->getAll();
            if (self::$container->load([Token::class => []])->Token->validateToken($data['csrftoken'])) {
                $this->model_instance->assign($data);
    ...
    public function verifyEmail($data)
    {
        if ($data) {
            $hash = array_pop($data);
            $id = array_pop($data);
            $request = self::$container->load([UsersRequestsManager::class => []])->UsersRequests->getDetails($id);
    ...
    {
        $tk = self::$container->load([Token::class => []])->Token;
            ...
            if ((int)$user[0]->verified === 0) {
                if ($user[1] <= MAX_PW_RESET_REQUESTS_PER_DAY) {
                    ...
                    if ($resp = $request->save()) {
                        if (H_Email::sendEmail($email, $user[0]->name, 'Email Verification', '<a href="http://localhost/kngell/auth/verifyEmail/' . $resp['saveID']->get_lastID() . '/' . $tk->urlSafeEncode($verifiCode) . '">Click to verify your email</a>')) {
                            return true;
                        }
                    }
                }
