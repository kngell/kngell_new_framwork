...
class AuthController extends Controller
{
    ...
    {
        if ($this->request->exists('post')) {
            if ($userdata = $this->container->make(AuthManager::class)->Auth->rememberMe_checker()) {
    ...
    public function ajaxRegister()
    {
        if ($this->request->exists('post')) {
            $data = $this->request->get();
            if ($this->token->validateToken($data['csrftoken'])) {
                $model = $this->container->make(AuthManager::class)->assign($data);
            ...
            if ((int)$user[0]->verified === 0) {
                if ($user[1] <= MAX_PW_RESET_REQUESTS_PER_DAY) {
                    ...
                    if ($resp = $request->save()) {
                        if (H_Email::sendEmail($email, $user[0]->name, 'Email Verification', '<a href="http://localhost/kngell/auth/verifyEmail/' . $resp['saveID']->get_lastID() . '/' . $this->token->urlSafeEncode($verifiCode) . '">Click to verify your email</a>')) {
                            return true;
                        }
                    }
                }
