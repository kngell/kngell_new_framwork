    ...
    <div class="container w-75">
        ...
        <div class="row">
            <div class="col-sm-9 cart_items" id="cart_items">
                ...
                <?php $products = array_filter($this->user_cart[0], function ($item) {return $item->c_content == 'cart';})?>
                <?php if (count($products) > 0) :?>
                <?php foreach ($products as $product) :?>
                <div class="row cart-row">
                    <div class="col-sm-2 cart-row__img">
                        <img src="<?=$product->p_media != '' ? IMG . unserialize($product->p_media)[0] : ImageManager::asset_img('products/product-80x80.jpg')?>"
                    ...
                    <div class="col-sm-8 cart-row__details">
                        ...
                        <div class="cart-qty">
                            <div class="qty-group">
                                <form class="form_qty">
                                    <?=FH::csrfInput('csrftoken', (new Token)->generate_token(8, 'form_qty' . $product->pdtID ?? 1)); ?>
                                    <input type="hidden" name="item_id"
                                        value="<?=$product->pdtID?>">
                                    <button class="qty-up border bg-light"><i class="fad fa-angle-up"></i></button>
                                    <input type="text" class="qty_input px-2 bg-light" name="qty"
                                        value="<?=$product->item_qty?>"
                                        placeholder="1" min="1">
                                    <button class="qty-down border bg-light"><i class="fad fa-angle-down"></i></button>
                                </form>
                            </div>
                            <form action="#"
                                class="delete-cart-item-frm<?=$product->pdtID?>">
                                <input type="hidden" name="item_id"
                                    value="<?=$product->pdtID?>">
                                <?=FH::csrfInput('csrftoken', (new Token())->generate_token(8, 'delete-cart-item-frm' . $product->pdtID ?? 1)); ?>
                                <button type="submit"
                                    class="btn font-baloo px-3 border-right deleteBtn">Supprimer</button>
                    ...
                    <div class="col-sm-2 text-right cart-row__price">
                        <div class="price_wrapper">
                            <span class="product_price"><?=$product->get_currency($product->p_regular_price * $product->item_qty)?></span>
                        </div>
                    </div>
                </div>
                <?php endforeach;?>
                <?php else :?>
                <?php echo $this->user_cart[4]?>
                <?php endif;?>
                <?php else:?>
                <?php echo $this->user_cart[4]?>
                <?php endif;?>
            ...
            <div class="col-sm-3 sub_total" id="sub_total">
                <div class="card sub-total mt-2 border">
                    <div class="card-header">
                        <p class="nb-item"><span class="cart_nb_elt"><?=$this->user_cart && is_array($this->user_cart[0]) ? count(array_filter($this->user_cart[0], function ($item) {return $item->c_content == 'cart';})) : 0?></span>&nbsp;<span>produits</span>
                        </p>
                    ...
                    <div class="card-footer proceed">
                        <form class="buy-frm">
                            <?=FH::csrfInput('csrftoken', (new Token)->generate_token(8, 'buy-frm')); ?>
